...
Environment: **Python 3.13.13** (notebook kernel `.venv`), CPU-only. The versions
below are the exact pins reported by `pip freeze` in the submission environment.

| Package | Version | Source |
| --- | --- | --- |
| `spotforecast2-safe` | 21.1.0 | pip freeze (submission env) |
| `spotoptim` | 1.0.0 | pip freeze (submission env) |
| `spotforecast2` | 7.0.0 | pip freeze (submission env) |
| `lightgbm` | 4.6.0 | pip freeze (submission env) |
| `pandas` | 3.0.3 | pip freeze (submission env) |
| `pyarrow` | 24.0.0 | pip freeze (submission env) |
| `numpy` | 2.4.6 | pip freeze (submission env) |
| `scikit-learn` | 1.9.0 | pip freeze (submission env) |
| `shap` | 0.52.0 | pip freeze (submission env) |
| `entsoe-py` | 0.8.0 | pip freeze (submission env) |

All entries are the installed versions. `spotforecast2-safe 21.1.0` and
`spotoptim 1.0.0` comfortably exceed the notebook's feature floors
(`spotforecast2-safe ≥ 16.4.0` for the target-corruption policy and day-ahead
providers; `spotoptim ≥ 0.12.7`).
...
> **Reproducing the determinism.** The deterministic profile can be reproduced
> and independently checked by assembling a frozen bundle — a frozen ENTSO-E
> snapshot, the pinned `uv.lock`, a deterministic reference CSV, and
> `SHA256SUMS` — and re-running the serial profile (`n_jobs_spotoptim=None`,
> `random_state=42`) against it, exactly as the reference `team_4` package does.
...
- **CR-1 (no dead code):** the `spotforecast2-safe` building blocks carry the
  ...
  end-to-end on every render with runtime asserts guarding each path; dedicated
  unit tests can be layered on top to lock this in.
- **CR-2 (determinism):** bit-for-bit reproducibility is available through the
  **deterministic archive profile** (§3, serial SpotOptim, `random_state=42`);
  the submitted forecast used the faster parallel profile, trading bit-identity
  for speed by design.
...
> **Enforcing the deny-list.** The environment separation above can be made
> machine-checkable by committing the `uv.lock` and adding a test that asserts
> none of the deny-listed packages appear in the inference lockfile — the
> reference package's audit step 1.
...
dependencies). This adds an automated, external supply-chain signal on top of the
environment separation in Dependency Governance above; committing a `uv.lock` and
adding the deny-list test are the natural next steps to make that separation
